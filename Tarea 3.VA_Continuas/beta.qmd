...
```{r}
#| label: exr-simulacion_beta-solucion
#| code-fold: true
#| fig-cap: "Convergencia de la Media y Varianza Muestral a los Valores Teóricos"
#| fig-width: 8
#| fig-height: 7

library(patchwork) # Para combinar gráficos

alpha_beta <- 2; beta_beta <- 5
media_teorica <- alpha_beta / (alpha_beta + beta_beta)
varianza_teorica <- (alpha_beta * beta_beta) / ((alpha_beta + beta_beta)^2 * (alpha_beta + beta_beta + 1))

n_sizes_conv <- seq(100, 10000, by = 100)
resultados_conv <- data.frame(n = n_sizes_conv, Media_Muestral = NA, Varianza_Muestral = NA)

set.seed(1)
for (i in 1:length(n_sizes_conv)) {
  n <- n_sizes_conv[i]
  muestra <- rbeta(n, shape1 = alpha_beta, shape2 = beta_beta)
  resultados_conv$Media_Muestral[i] <- mean(muestra)
  resultados_conv$Varianza_Muestral[i] <- var(muestra)
}

# 1. Gráfica de convergencia de la Media
plot1 <- ggplot(resultados_conv, aes(x = n)) +
  geom_line(aes(y = Media_Muestral), color = "blue", alpha = 0.7) +
  geom_hline(yintercept = media_teorica, linetype = "dashed", color = "red") +
  labs(title = "Convergencia de la Media Muestral",
       subtitle = paste0("Teórica: ", round(media_teorica, 4)),
       x = "Tamaño de la Muestra (n)", y = "Media Muestral") +
  theme_minimal()

# 2. Gráfica de convergencia de la Varianza
plot2 <- ggplot(resultados_conv, aes(x = n)) +
  geom_line(aes(y = Varianza_Muestral), color = "green", alpha = 0.7) +
  geom_hline(yintercept = varianza_teorica, linetype = "dashed", color = "red") +
  labs(title = "Convergencia de la Varianza Muestral",
       subtitle = paste0("Teórica: ", round(varianza_teorica, 5)),
       x = "Tamaño de la Muestra (n)", y = "Varianza Muestral") +
  theme_minimal()

# Combinar gráficos de convergencia
plot_conv <- plot1 / plot2
print(plot_conv)

# 3. Histograma de la muestra más grande (n=10000)
n_max <- max(n_sizes_conv)
set.seed(2)
muestra_max <- rbeta(n_max, shape1 = alpha_beta, shape2 = beta_beta)
data_hist <- data.frame(X = muestra_max)

plot3 <- ggplot(data_hist, aes(x = X)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50, fill = "purple", alpha = 0.6, color = "black") +
  stat_function(fun = dbeta, args = list(shape1 = alpha_beta, shape2 = beta_beta),
                color = "red", linewidth = 1) +
  labs(title = paste0("Distribución $\\text{Beta}(2, 5)$ con n=", n_max),
       subtitle = "Curva teórica (roja) superpuesta",
       x = "Probabilidad (X)", y = "Densidad") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))

cat("\n[Gráfico de Histograma para n=10000]\n")
print(plot3)

cat("--- Resultado Ejercicio 1: Resumen ---\n")
cat("Media Teórica E(X):", round(media_teorica, 4), "\n")
cat("Varianza Teórica Var(X):", round(varianza_teorica, 5), "\n")
```
...
```{r}
#| label: exr-tcl_beta-solucion
#| code-fold: true
#| fig-cap: "Verificación del Teorema Central del Límite (TCL) para $\text{Beta}(2, 5)$"
#| fig-width: 8
#| fig-height: 7

library(patchwork) # Para combinar gráficos

k_sim_tcl <- 1000 # Número de medias muestrales
n_sizes_tcl <- c(5, 15, 30, 60) # Tamaños de muestra

cat("--- Resultado Ejercicio 2: Teorema Central del Límite ---\n")

results_tcl_beta <- list()
plots_tcl <- list()

# Bucle para cada tamaño de muestra (n)
for (n in n_sizes_tcl) {
  set.seed(n) 
  medias <- replicate(k_sim_tcl, {
    mean(rbeta(n, shape1 = alpha_beta, shape2 = beta_beta))
  })
  
  # Parámetros teóricos de la media muestral
  media_tcl_teorica <- media_teorica
  varianza_tcl_teorica_media <- varianza_teorica / n
  desv_std_tcl_teorica_media <- sqrt(varianza_tcl_teorica_media)
  
  results_tcl_beta[[as.character(n)]] <- data.frame(
    n = n,
    Media_Obs = mean(medias),
    Varianza_Obs = var(medias),
    Varianza_Teorica_Media = varianza_tcl_teorica_media
  )
  
  # Gráfico de Histograma y Curva Normal
  data_plot_tcl <- data.frame(Medias = medias)
  plots_tcl[[as.character(n)]] <- ggplot(data_plot_tcl, aes(x = Medias)) +
    geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "lightblue", alpha = 0.7, color = "black") +
    # Curva Normal Teórica (por el TCL)
    stat_function(fun = dnorm, args = list(mean = media_tcl_teorica, sd = desv_std_tcl_teorica_media),
                  color = "red", linetype = "dashed", linewidth = 1) +
    labs(title = paste("n =", n), x = "Media Muestral", y = "Densidad") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5))
}

# Unir tabla de resultados
tabla_tcl <- bind_rows(results_tcl_beta)

cat("Tabla de Parámetros (Verificación TCL):\n")
print(tabla_tcl %>% kable(caption = "Comparación de Parámetros de Medias Muestrales", digits = 4) %>% kable_styling())

# Organizar gráficos
plot_final_tcl <- wrap_plots(plots_tcl, ncol = 2) + 
  plot_annotation(title = 'Teorema Central del Límite para $\\text{Beta}(2, 5)$')

print(plot_final_tcl)
```
