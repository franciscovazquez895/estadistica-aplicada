...
analizar_convergencia <- function(grados_libertad, n_max = 10000) {
  # Medidas teóricas
  media_teorica <- grados_libertad
  varianza_teorica <- 2 * grados_libertad

  # 1. Generar la muestra completa
  set.seed(123) # Para reproducibilidad
  muestra <- rchisq(n_max, df = grados_libertad)

  # 2. Calcular convergencia
  indices <- 1:n_max
  medias_acumuladas <- cumsum(muestra) / indices
  varianzas_acumuladas <- sapply(indices, function(i) {
    if (i == 1) return(NA)
    var(muestra[1:i])
  })
  ...
  df_convergencia <- data.frame(
    ...
    varianza = varianzas_acumuladas
  )

  # --- Gráfica de convergencia de la media ---
  p_media <- ggplot(df_convergencia, aes(x = n, y = media)) +
    geom_line(color = "dodgerblue", linewidth = 1) +
    geom_hline(yintercept = media_teorica, color = "red", linetype = "dashed", linewidth = 1.2) +
    labs(
      title = paste("Convergencia de la Media (n =", grados_libertad, ")"),
      subtitle = paste("Media teórica (rojo) =", media_teorica),
      x = "Tamaño de Muestra", y = "Media Muestral"
    ) +
    theme_minimal()

  # --- Gráfica de convergencia de la varianza ---
  p_varianza <- ggplot(df_convergencia, aes(x = n, y = varianza)) +
    geom_line(color = "forestgreen", linewidth = 1) +
    geom_hline(yintercept = varianza_teorica, color = "red", linetype = "dashed", linewidth = 1.2) +
    labs(
      title = paste("Convergencia de la Varianza (n =", grados_libertad, ")"),
      subtitle = paste("Varianza teórica (rojo) =", varianza_teorica),
      x = "Tamaño de Muestra", y = "Varianza Muestral"
    ) +
    theme_minimal()

  # --- Histograma vs Densidad Teórica (para n_max) ---
  p_hist <- ggplot(data.frame(x = muestra), aes(x = x)) +
    geom_histogram(aes(y = after_stat(density)), bins = 50,
                   fill = "lightblue", color = "black", alpha = 0.8) +
    stat_function(
      fun = dchisq, args = list(df = grados_libertad),
      color = "red", linewidth = 1.5
    ) +
    labs(
      title = paste("Histograma vs Densidad Teórica (n =", grados_libertad, ")"),
      subtitle = paste("Muestra de", n_max, "observaciones"),
      x = "Valor", y = "Densidad"
    ) +
    theme_minimal()

  # Imprimir los gráficos
  grid.arrange(p_media, p_varianza, p_hist, ncol = 3, top = paste("Análisis para χ²(", grados_libertad, ")"))
}

# --- Ejecutar análisis para n = 5, 20 y 50 ---
analizar_convergencia(grados_libertad = 5)
analizar_convergencia(grados_libertad = 20)
analizar_convergencia(grados_libertad = 50)
```
...
grados_libertad <- 8  # Podemos elegir cualquier valor
tamano_muestra <- 30 # Tamaño de cada muestra para calcular la media
num_simulaciones <- 5000 # Número de veces que repetimos el muestreo

# --- Parámetros teóricos de la distribución de medias muestrales (según TCL) ---
media_teorica_ji <- grados_libertad
varianza_teorica_ji <- 2 * grados_libertad

# La media de las medias muestrales es la media de la población original
media_de_medias <- media_teorica_ji
# La desviación estándar de las medias muestrales (error estándar)
sd_de_medias <- sqrt(varianza_teorica_ji / tamano_muestra)

# --- Realizar la simulación ---
set.seed(456)
# Usamos 'replicate' para repetir la operación y obtener un vector de medias
medias_muestrales <- replicate(num_simulaciones, {
  muestra <- rchisq(tamano_muestra, df = grados_libertad)
  mean(muestra)
})

# --- Visualización ---
ggplot(data.frame(medias = medias_muestrales), aes(x = medias)) +
  geom_histogram(aes(y = after_stat(density)), bins = 40,
                 fill = "orange", color = "black", alpha = 0.7) +
  # Superponer la curva normal teórica predicha por el TCL
  stat_function(
    fun = dnorm,
    args = list(mean = media_de_medias, sd = sd_de_medias),
    color = "blue",
    linewidth = 1.5,
    linetype = "dashed"
  ) +
  labs(
    title = "Verificación del Teorema Central del Límite",
    subtitle = paste("Distribución de", num_simulaciones, "medias muestrales de tamaño", tamano_muestra, "de una χ²(", grados_libertad, ")"),
    x = "Media Muestral",
    y = "Densidad",
    caption = "La curva azul es la distribución Normal teórica predicha por el TCL."
  ) +
  theme_minimal()
```
...
# --- Parámetros de la simulación ---
grados_libertad <- 5   # Número de variables N(0,1) a sumar
num_simulaciones <- 10000 # Número de veces que realizamos la suma

# --- Realizar la simulación ---
set.seed(789)
# 'replicate' repite la operación de generar 'n' normales, elevarlas al cuadrado y sumarlas
sumas_cuadradas <- replicate(num_simulaciones, {
  sum(rnorm(grados_libertad)^2)
})

# --- Visualización ---
ggplot(data.frame(x = sumas_cuadradas), aes(x = x)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50,
                 fill = "cyan", color = "black", alpha = 0.7) +
  # Superponer la curva teórica de la Ji-cuadrada con n grados de libertad
  stat_function(
    fun = dchisq,
    args = list(df = grados_libertad),
    color = "red",
    linewidth = 1.5
  ) +
  labs(
    title = "Suma de Cuadrados de Normales Estándar",
    subtitle = paste("Simulación de Σ Zᵢ² vs. la distribución teórica χ²(", grados_libertad, ")"),
    x = "Valor de la suma",
    y = "Densidad",
    caption = "La curva roja es la densidad teórica de la χ²."
  ) +
  theme_minimal()
```
...
# --- Función para comparar χ² con la Normal ---
comparar_aproximacion_normal <- function(grados_libertad, num_muestras = 10000) {
  # Generar muestra de la Ji-cuadrada
  set.seed(101)
  muestra_ji <- rchisq(num_muestras, df = grados_libertad)

  # Parámetros de la Normal teórica aproximada
  media_normal_aprox <- grados_libertad
  sd_normal_aprox <- sqrt(2 * grados_libertad)

  # Gráfico
  ggplot(data.frame(x = muestra_ji), aes(x = x)) +
    geom_histogram(aes(y = after_stat(density)), bins = 50,
                   fill = "lightpink", color = "black", alpha = 0.7) +
    # Curva teórica Ji-cuadrada (real)
    stat_function(
      fun = dchisq, args = list(df = grados_libertad),
      aes(color = "Ji-cuadrada Teórica"), linewidth = 1.5
    ) +
    # Curva teórica Normal (aproximación)
    stat_function(
      fun = dnorm, args = list(mean = media_normal_aprox, sd = sd_normal_aprox),
      aes(color = "Aproximación Normal"), linewidth = 1.5, linetype = "dashed"
    ) +
    scale_color_manual(name = "Distribución", values = c("Ji-cuadrada Teórica" = "red", "Aproximación Normal" = "blue")) +
    labs(
      title = paste("Aproximación Normal para n =", grados_libertad),
      x = "Valor", y = "Densidad"
    ) +
    theme_minimal() +
...
# --- Generar gráficos para diferentes n ---
p10 <- comparar_aproximacion_normal(10)
p30 <- comparar_aproximacion_normal(30)
p50 <- comparar_aproximacion_normal(50)
p100 <- comparar_aproximacion_normal(100)

# Mostrar todos juntos
grid.arrange(p10, p30, p50, p100, ncol = 2)
```
...
# --- Parámetros de la simulación ---
n1 <- 4  # Grados de libertad de la primera variable
n2 <- 6  # Grados de libertad de la segunda variable
num_simulaciones <- 10000

# Grados de libertad de la distribución resultante teórica
n_suma <- n1 + n2

# --- Realizar la simulación ---
set.seed(202)
# Generar muestras de cada distribución
muestra1 <- rchisq(num_simulaciones, df = n1)
muestra2 <- rchisq(num_simulaciones, df = n2)

# Sumar las muestras
suma_muestras <- muestra1 + muestra2

# --- Visualización ---
ggplot(data.frame(x = suma_muestras), aes(x = x)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50,
                 fill = "mediumpurple", color = "black", alpha = 0.7) +
  # Superponer la curva teórica de la Ji-cuadrada con n_suma grados de libertad
  stat_function(
    fun = dchisq,
    args = list(df = n_suma),
    color = "darkgreen",
  ...
  labs(
    title = "Verificación de la Propiedad de Aditividad",
    subtitle = paste("Suma de χ²(", n1, ") + χ²(", n2, ") vs. la teórica χ²(", n_suma, ")"),
    x = "Valor de la suma",
    y = "Densidad",
    caption = "La curva verde es la densidad teórica de la χ² resultante."
  ) +
  theme_minimal()
```
