...
# --- Valores Teóricos ---
# Media teórica para t(5) es 0, porque n > 1 [cite: 33]
media_teorica <- 0
# Varianza teórica para t(5) es n/(n-2), porque n > 2 [cite: 34]
varianza_teorica <- grados_libertad / (grados_libertad - 2)

# --- 1. Calcular media y varianza muestral ---
resultados <- data.frame()

for (n in tamanos_muestra) {
  set.seed(42) # Para reproducibilidad
  muestra <- rt(n, df = grados_libertad)
...
kable(resultados, digits = 4, caption = "Comparación de Estadísticos Muestrales vs. Teóricos para t(5)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

# --- 2. Histograma para la muestra más grande (n=1000) ---
set.seed(42)
muestra_grande <- rt(1000, df = grados_libertad)

ggplot(data.frame(x = muestra_grande), aes(x = x)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30,
                 fill = "lightblue", color = "black", alpha = 0.7) +
  # Superponer la densidad teórica de t(5)
  stat_function(
    fun = dt, args = list(df = grados_libertad),
    aes(color = "t(5) Teórica"), linewidth = 1.5
  ) +
  # Superponer la densidad de la normal estándar
  stat_function(
    fun = dnorm, args = list(mean = 0, sd = 1),
    aes(color = "Normal Estándar N(0,1)"), linewidth = 1.5, linetype = "dashed"
  ) +
  scale_color_manual(name = "Función de Densidad", values = c("t(5) Teórica" = "red", "Normal Estándar N(0,1)" = "blue")) +
  labs(
    title = "Histograma de Muestra t(5) vs. Densidades Teóricas",
    subtitle = "Muestra de n=1000. Se observa que t(5) tiene colas más pesadas que la N(0,1).",
    x = "Valor",
    y = "Densidad"
  ) +
  theme_minimal() +
...
# --- Generar muestras de las distribuciones base ---
set.seed(123)
# Muestra de una Normal Estándar N(0,1)
X <- rnorm(n_sim, mean = 0, sd = 1)

# Muestra de una Ji-cuadrada con 10 grados de libertad
Y <- rchisq(n_sim, df = grados_libertad)

# --- Calcular la variable T según la fórmula ---
T_calculada <- X / sqrt(Y / grados_libertad)

# --- Comparar histograma con la densidad teórica de t(10) ---
ggplot(data.frame(t_val = T_calculada), aes(x = t_val)) +
  geom_histogram(aes(y = after_stat(density)), bins = 40,
                 fill = "lightgreen", color = "black", alpha = 0.8) +
  # Superponer la densidad teórica de t(10)
  stat_function(
    fun = dt,
  ...
  labs(
    title = "Construcción de una Variable t de Student",
    subtitle = paste("Histograma de T = X / sqrt(Y/n) vs. la densidad teórica de t(", grados_libertad, ")"),
    x = "Valor de T calculado",
    y = "Densidad",
    caption = "La curva roja es la densidad teórica de t(10), que se ajusta perfectamente a la simulación."
  ) +
  theme_minimal()
```
...
# --- Parámetros de la simulación ---
num_simulaciones <- 1000 # Número de muestras a generar
tamano_muestra <- 10      # Tamaño de cada muestra individual
mu_poblacional <- 0       # Media de la población Normal de origen

# Los grados de libertad de la distribución t resultante serán n-1
grados_libertad_t <- tamano_muestra - 1

# --- Realizar la simulación ---
set.seed(789)
# Usamos 'replicate' para repetir el proceso de muestreo y cálculo
t_estadisticos <- replicate(num_simulaciones, {
  # 1. Generar una muestra de tamaño 10 de una N(0,1)
  muestra_normal <- rnorm(tamano_muestra, mean = mu_poblacional, sd = 1)
  
  # 2. Calcular la media y desviación estándar de esa muestra
  x_barra <- mean(muestra_normal)
  s <- sd(muestra_normal)
  
  # 3. Calcular el estadístico T para esa muestra
  t_calculado <- (x_barra - mu_poblacional) / (s / sqrt(tamano_muestra))
  
  # El valor retornado se almacena en el vector 't_estadisticos'
  return(t_calculado)
})

# --- Comparar el histograma de los estadísticos t con la densidad teórica de t(9) ---
ggplot(data.frame(t = t_estadisticos), aes(x = t)) +
  geom_histogram(aes(y = after_stat(density)), bins = 40,
                 fill = "orange", color = "black", alpha = 0.7) +
  # Superponer la densidad teórica de t(9)
  stat_function(
    fun = dt,
    args = list(df = grados_libertad_t),
    color = "purple",
    linewidth = 1.5
  ) +
  labs(
    title = "Distribución Muestral del Estadístico t",
    subtitle = paste("Histograma de 1000 estadísticos t calculados vs. la densidad teórica de t(", grados_libertad_t, ")"),
    x = "Valor del Estadístico t calculado",
    y = "Densidad",
    caption = "La curva púrpura es la densidad teórica de t(9)."
  ) +
  theme_minimal()
```
