...
```{r}
#| label: exr-tiempo_carga-solucion
#| code-fold: true
...
cat("--- Resultado Ejercicio 1: Tiempo de Carga ---\n")

# 1. Histograma con Densidad Teórica
set.seed(123)
tiempos_carga <- runif(n_sim1, min = a1, max = b1)
data_ex1 <- data.frame(Tiempo = tiempos_carga)

plot1 <- ggplot(data_ex1, aes(x = Tiempo)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, 
                 fill = "steelblue", alpha = 0.6, color = "black") +
  # Densidad teórica f(x) = 1/(b-a)
  geom_rect(xmin = a1, xmax = b1, ymin = 0, ymax = 1/(b1 - a1), 
            fill = "red", alpha = 0.2, color = "red", linetype = "dashed") +
  labs(title = "Distribución de Tiempos de Carga Simulados",
       subtitle = paste0("Teórica: Uniforme(", a1, ", ", b1, ")"),
       x = "Tiempo (segundos)",
       y = "Densidad") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
...
# 2. Percentil 75 (Q3)
percentil75 <- qunif(p = 0.75, min = a1, max = b1)
cat("2. Percentil 75 (Q3):", percentil75, "segundos\n")

# 3. P(X < 4)
prob_p4 <- punif(q = 4, min = a1, max = b1)
cat("3. P(X < 4 segundos):", round(prob_p4, 4), "\n")
```
...
`
```{r}
#| label: exr-calificaciones-solucion
#| code-fold: true
...
cat("--- Resultado Ejercicio 2: Calificaciones ---\n")

# 1. Simular calificaciones
set.seed(456)
calificaciones <- runif(n_sim2, min = a2, max = b2)

# 2. Porcentaje superior a 85
porc_sup85_sim <- mean(calificaciones > 85)
porc_sup85_teorico <- 1 - punif(q = 85, min = a2, max = b2)
cat("2. Porcentaje teórico (> 85):", scales::percent(porc_sup85_teorico), "\n")
cat("   Porcentaje simulado (> 85):", scales::percent(porc_sup85_sim), "\n")

# 3. y 4. Categorías y comparación
# Ajustamos breaks para incluir 100
categorias <- cut(calificaciones, 
                  breaks = c(70, 80, 90, 100.0001), 
                  right = FALSE, 
                  labels = c("Aprobado (70-79)", "Bien (80-89)", "Excelente (90-100)"))
frec_sim <- prop.table(table(categorias))

# Porcentajes teóricos: ancho de cada intervalo / ancho total (30)
p_aprobado <- 10 / (b2-a2) 
p_bien <- 10 / (b2-a2)     
p_excelente <- 10 / (b2-a2) 

comparacion_cat <- data.frame(
  Categoría = c("Aprobado (70-79)", "Bien (80-89)", "Excelente (90-100)"),
  Teórico = c(p_aprobado, p_bien, p_excelente),
  Simulado = as.numeric(frec_sim)
)

cat("3. y 4. Comparación de Porcentajes por Categoría:\n")
print(comparacion_cat %>% 
        kable(caption = "Porcentajes por Categoría", digits = 4) %>% 
...
plot2 <- ggplot(data_categorias, aes(x = Calificacion, fill = Categoria)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, color = "black", alpha = 0.7) +
  geom_rect(xmin = a2, xmax = b2, ymin = 0, ymax = 1/(b2-a2), 
            fill = NA, color = "red", linetype = "dashed", linewidth = 1) +
  labs(title = "Distribución de Calificaciones (Simulación)",
       subtitle = "Uniforme Teórica [70, 100]",
       x = "Calificación",
       y = "Densidad") +
...
```{r}
#| label: exr-tcl_verificacion-solucion
#| code-fold: true
#| fig-cap: "Distribución de Medias Muestrales para Uniforme Asimétrica (Verificación del TCL)"
#| fig-width: 10
#| fig-height: 7

# 1. Función para generar la distribución sesgada: Uniforme(0,1) truncada en > 0.3
generate_sesgada <- function(n) {
  x <- numeric(n)
  i <- 1
  while (i <= n) {
...
# 2. Generar y analizar medias muestrales para n = c(5, 15, 30, 60, 120)
n_sizes <- c(5, 15, 30, 60, 120)
k_sim3 <- 800
results_tcl <- list()

cat("--- Resultado Ejercicio 3: Verificación del TCL ---\n")

for (n in n_sizes) {
  medias <- replicate(k_sim3, {
    mean(generate_sesgada(n))
  })
  
  # Almacenar resultados
  results_tcl[[as.character(n)]] <- data.frame(Medias = medias, n = factor(n))
}

# Unir resultados para el gráfico
data_tcl <- bind_rows(results_tcl)

plot3 <- ggplot(data_tcl, aes(x = Medias)) +
  ...
  labs(title = "Distribución de Medias Muestrales (Uniforme Asimétrica)",
       subtitle = "Verificación del TCL para diferentes tamaños de muestra (n)",
       x = "Media Muestral",
       y = "Densidad") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
...
# 3. Análisis visual
cat("3. Conclusión visual:\n")
cat("   - Para n=5 y n=15, la distribución de medias aún es notablemente sesgada.\n")
cat("   - A partir de n=30, la distribución comienza a mostrar una forma simétrica y acampanada (Normal).\n")
cat("   - Para n=60 y n=120, la aproximación a la Normal es muy buena.\n")
```
...
```{r}
#| label: exr-pi_simulacion-solucion
#| code-fold: true
#| fig-cap: "Simulación de Monte Carlo para la estimación de Pi (100,000 puntos)"
#| fig-width: 6
#| fig-height: 6

n_sim4 <- 100000 # Número de puntos

cat("--- Resultado Ejercicio 4: Simulación Monte Carlo (Pi) ---\n")

# Generar puntos aleatorios (x, y) en el cuadrado [0, 1] x [0, 1]
set.seed(789)
x <- runif(n_sim4, min = 0, max = 1)
y <- runif(n_sim4, min = 0, max = 1)

# Verificar si el punto cae dentro del círculo de radio 1 (x^2 + y^2 <= 1)
dentro_circulo <- (x^2 + y^2) <= 1
n_dentro <- sum(dentro_circulo)

# Pi/4 = P(dentro del círculo) = n_dentro / n_sim4
pi_estimado <- 4 * (n_dentro / n_sim4)

# Data frame para el gráfico (usando una muestra más pequeña para visualización clara)
data_pi <- data.frame(x = x[1:1000], y = y[1:1000], 
                      Dentro = dentro_circulo[1:1000])

plot4 <- ggplot(data_pi, aes(x = x, y = y, color = Dentro)) +
  geom_point(size = 0.5) +
  coord_fixed() +
  geom_path(data = data.frame(t = seq(0, pi/2, length.out = 100), 
                             x = cos(seq(0, pi/2, length.out = 100)), 
                             y = sin(seq(0, pi/2, length.out = 100))), 
            aes(x = x, y = y), inherit.aes = FALSE, color = "red", linewidth = 1) +
  labs(title = "Estimación de Pi por Monte Carlo",
       subtitle = paste("Estimación:", round(pi_estimado, 5), " | Puntos:", n_sim4),
       x = "Eje X",
       y = "Eje Y") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
...
cat("Pi estimado por Monte Carlo con", n_sim4, "puntos:", round(pi_estimado, 5), "\n")
```
