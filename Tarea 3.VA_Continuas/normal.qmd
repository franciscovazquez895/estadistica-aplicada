...
mu_temp <- 350
sigma_temp <- 5
limite_inferior <- 340 # 350 - 10
limite_superior <- 360 # 350 + 10

# --- 1. Simular un día de producción ---
set.seed(456)
mediciones_temp <- rnorm(1000, mean = mu_temp, sd = sigma_temp)
cat("Se han simulado 1000 mediciones de temperatura.\n\n")

# --- 2. Porcentaje fuera de especificación (Teórico) ---
prob_debajo <- pnorm(limite_inferior, mean = mu_temp, sd = sigma_temp)
prob_arriba <- pnorm(limite_superior, mean = mu_temp, sd = sigma_temp, lower.tail = FALSE)
prob_fuera_total <- (prob_debajo + prob_arriba) * 100

cat("--- Calidad del Proceso Centrado ---\n")
cat("Porcentaje teórico de mediciones fuera de especificación [340, 360]:", round(prob_fuera_total, 4), "%\n\n")

# --- 3. Cambio en la calidad si el proceso se descentra ---
mu_descentrado <- 355
prob_debajo_desc <- pnorm(limite_inferior, mean = mu_descentrado, sd = sigma_temp)
prob_arriba_desc <- pnorm(limite_superior, mean = mu_descentrado, sd = sigma_temp, lower.tail = FALSE)
prob_fuera_total_desc <- (prob_debajo_desc + prob_arriba_desc) * 100

cat("--- Calidad del Proceso Descentrado (μ = 355) ---\n")
cat("Nuevo porcentaje teórico fuera de especificación:", round(prob_fuera_total_desc, 4), "%\n\n")

# --- 4. ¿Qué sigma se necesita para tener 99.9% de calidad? ---
# El 99.9% de los productos debe estar dentro de μ ± 10°C.
# Esto deja 0.05% en cada cola (0.0005).
# Buscamos el valor z para una probabilidad acumulada de 1 - 0.0005 = 0.9995
z_critico <- qnorm(0.9995)

# La fórmula es: Límite Superior = μ + z * σ_nuevo
# 360 = 350 + z_critico * σ_nuevo
sigma_necesario <- (limite_superior - mu_temp) / z_critico

cat("--- Mejora del Proceso ---\n")
cat("Para lograr una calidad del 99.9%, la desviación estándar debe ser:", round(sigma_necesario, 4), "°C\n")
```
...
# --- 1. Simulación de ventas de 24 meses ---
set.seed(789)
ventas_simuladas <- rnorm(24, mean = mu_ventas, sd = sigma_ventas)
cat("--- Simulación de 24 Meses ---\n")
print(round(ventas_simuladas, 2))
cat("\n")

# --- 2. Probabilidad de ventas > 60,000 pesos (Teórico) ---
prob_sup_60k <- pnorm(60000, mean = mu_ventas, sd = sigma_ventas, lower.tail = FALSE)
cat("--- Cálculos Teóricos ---\n")
cat("Probabilidad de tener ventas > $60,000:", round(prob_sup_60k, 4), "\n")

# --- 3. Meses esperados con ventas < 30,000 pesos ---
prob_inf_30k <- pnorm(30000, mean = mu_ventas, sd = sigma_ventas)
meses_esperados <- prob_inf_30k * 24 # En un periodo de 24 meses
cat("En 24 meses, se espera tener ventas < $30,000 en aprox.:", round(meses_esperados, 1), "meses\n")

# --- 4. Rango Intercuartílico (IQR) (Teórico) ---
q1 <- qnorm(0.25, mean = mu_ventas, sd = sigma_ventas)
q3 <- qnorm(0.75, mean = mu_ventas, sd = sigma_ventas)
iqr <- q3 - q1
cat("El rango intercuartílico teórico es:", round(iqr, 2), "pesos\n")

# --- 5. Porcentaje de meses que se cumple la meta > 45,000 ---
prob_meta <- pnorm(45000, mean = mu_ventas, sd = sigma_ventas, lower.tail = FALSE)
porcentaje_meta <- prob_meta * 100
cat("El porcentaje de meses que se cumple la meta de > $45,000 es:", round(porcentaje_meta, 2), "%\n")
```
...
# --- 1. Simulación de la vida útil de 800 baterías ---
set.seed(101)
vida_baterias <- rnorm(800, mean = mu_bateria, sd = sigma_bateria)
cat("Se ha simulado la vida útil de 800 baterías.\n\n")

# --- 2. Garantía que cubre al 95% de las baterías ---
# Esto corresponde al percentil 5, ya que el 95% durará MÁS que este valor.
garantia_95 <- qnorm(0.05, mean = mu_bateria, sd = sigma_bateria)
cat("--- Garantías y Fallos ---\n")
cat("Una garantía de", round(garantia_95, 0), "horas cubriría al 95% de las baterías.\n")

# --- 3. Baterías que fallan con garantía de 900 horas ---
prob_fallo <- pnorm(900, mean = mu_bateria, sd = sigma_bateria)
baterias_fallan <- prob_fallo * 10000
cat("Con una garantía de 900 horas, se espera que fallen", round(baterias_fallan, 0), "de 10,000 baterías.\n\n")

# --- 4. Comparación de media y mediana (simulación vs. teórico) ---
media_sim <- mean(vida_baterias)
mediana_sim <- median(vida_baterias)
cat("--- Comparación de Parámetros ---\n")
cat("Media Teórica:", mu_bateria, "| Media Simulación:", round(media_sim, 2), "\n")
cat("Mediana Teórica:", mu_bateria, "| Mediana Simulación:", round(mediana_sim, 2), "\n\n")

# --- 5. Percentil 10 de la distribución (Teórico) ---
percentil_10 <- qnorm(0.10, mean = mu_bateria, sd = sigma_bateria)
cat("--- Percentiles ---\n")
cat("El percentil 10 de la duración de las baterías es:", round(percentil_10, 2), "horas.\n")
```
...
# --- 1. Simulación de 1000 mediciones ---
set.seed(202)
errores_simulados <- rnorm(1000, mean = mu_error, sd = sigma_error)
cat("Se han simulado 1000 errores de medición.\n\n")

# --- 2. Porcentaje de errores entre -5 y +5 unidades (Teórico) ---
# Esto es P(-5 < X < 5) = F(5) - F(-5)
prob_intervalo <- pnorm(5, mu_error, sigma_error) - pnorm(-5, mu_error, sigma_error)
porcentaje_intervalo <- prob_intervalo * 100
cat("--- Análisis de Precisión ---\n")
cat("El porcentaje teórico de errores entre -5 y +5 es:", round(porcentaje_intervalo, 2), "%\n")
# Esto corresponde a μ ± 2σ, por lo que debería ser cercano al 95%

# --- 3. Probabilidad de error > 6 en valor absoluto ---
# P(|X| > 6) = P(X > 6) + P(X < -6)
prob_abs_6 <- pnorm(6, mu_error, sigma_error, lower.tail = FALSE) + pnorm(-6, mu_error, sigma_error)
cat("La probabilidad de que un error sea > 6 en valor absoluto es:", round(prob_abs_6, 5), "\n")

# --- 4. Porcentaje de mediciones "precisas" (error <= 3) ---
# P(|X| <= 3) = P(-3 < X < 3)
prob_precisa <- pnorm(3, mu_error, sigma_error) - pnorm(-3, mu_error, sigma_error)
porcentaje_preciso <- prob_precisa * 100
cat("El porcentaje de mediciones consideradas 'precisas' es:", round(porcentaje_preciso, 2), "%\n\n")

# --- 5. Verificación de la simetría ---
media_error_sim <- mean(errores_simulados)
mediana_error_sim <- median(errores_simulados)
cat("--- Verificación de Simetría ---\n")
cat("La media de la simulación es:", round(media_error_sim, 4), "(Teórica: 0)\n")
cat("La mediana de la simulación es:", round(mediana_error_sim, 4), "(Teórica: 0)\n")
cat("Al ser casi idénticas, se confirma la simetría.\n")
```
...
# --- 1. Simulación de 200 estudiantes de cada grupo ---
set.seed(303)
grupo_A <- rnorm(200, mean = mu_A, sd = sigma_A)
grupo_B <- rnorm(200, mean = mu_B, sd = sigma_B)
cat("Se han simulado 200 estudiantes para cada grupo.\n\n")

# --- 2. Porcentaje de estudiantes del Grupo A que superan la media del Grupo B (Teórico) ---
prob_A_sup_mediaB <- pnorm(mu_B, mean = mu_A, sd = sigma_A, lower.tail = FALSE)
porcentaje_A_sup_mediaB <- prob_A_sup_mediaB * 100
cat("--- Comparaciones Teóricas ---\n")
cat("Porcentaje del Grupo A que supera la media del Grupo B (82 pts):", round(porcentaje_A_sup_mediaB, 2), "%\n")

# --- 3. Probabilidad de que un estudiante del Grupo B tenga puntuación < 70 ---
prob_B_inf_70 <- pnorm(70, mean = mu_B, sd = sigma_B)
cat("Probabilidad de que un estudiante del Grupo B obtenga < 70 puntos:", round(prob_B_inf_70, 4), "\n\n")

# --- 5. Puntuación del 10% superior de cada grupo (Percentil 90) ---
p90_A <- qnorm(0.90, mean = mu_A, sd = sigma_A)
p90_B <- qnorm(0.90, mean = mu_B, sd = sigma_B)
cat("--- Puntuaciones de Corte (Percentil 90) ---\n")
cat("El 10% superior del Grupo A tiene una puntuación de", round(p90_A, 1), "o más.\n")
cat("El 10% superior del Grupo B tiene una puntuación de", round(p90_B, 1), "o más.\n\n")

# --- 4. Comparación visual de las distribuciones ---
# Crear un dataframe para ggplot
df_A <- data.frame(puntuacion = grupo_A, grupo = "Grupo A")
df_B <- data.frame(puntuacion = grupo_B, grupo = "Grupo B")
df_combinado <- rbind(df_A, df_B)

ggplot(df_combinado, aes(x = puntuacion, fill = grupo)) +
  geom_density(alpha = 0.6) +
  labs(
    title = "Comparación de Distribuciones de Puntuaciones",
    subtitle = "Grupo A: N(75, 10²) vs. Grupo B: N(82, 8²)",
    x = "Puntuación del Examen",
    y = "Densidad",
    fill = "Grupos"
  ) +
  geom_vline(xintercept = mu_A, color = "red", linetype = "dashed") +
  geom_vline(xintercept = mu_B, color = "blue", linetype = "dashed") +
  annotate("text", x = mu_A, y = 0.045, label = "Media A", color = "red", angle = 90, vjust = -0.5) +
  annotate("text", x = mu_B, y = 0.045, label = "Media B", color = "blue", angle = 90, vjust = -0.5) +
  theme_minimal()
```
