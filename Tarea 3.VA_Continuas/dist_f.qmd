...
n_grados_libertad <- 15 # Grados de libertad para la t de Student
num_simulaciones <- 20000

# --- Simulación ---
set.seed(123) # Para reproducibilidad
# 1. Generar una muestra de una distribución t(n)
muestra_t <- rt(num_simulaciones, df = n_grados_libertad)

# 2. Elevar al cuadrado cada valor de la muestra
muestra_t_cuadrada <- muestra_t^2

# --- Visualización y Comparación ---
# 3. Graficar el histograma de la muestra transformada
ggplot(data.frame(x = muestra_t_cuadrada), aes(x = x)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50,
                 fill = "skyblue", color = "black", alpha = 0.7) +
  # 4. Superponer la PDF teórica de F(1, n)
  stat_function(
    fun = df,
    args = list(df1 = 1, df2 = n_grados_libertad),
    color = "red",
    linewidth = 1.5
  ) +
  labs(
    title = "Verificación: X ~ t(n) => X² ~ F(1, n)",
    subtitle = paste("Simulación para n =", n_grados_libertad, ". La curva roja es la PDF teórica de F(1, 15)."),
    x = "Valor de X²",
    y = "Densidad"
  ) +
  theme_minimal() +
  coord_cartesian(xlim = c(0, 10)) # Acotar el eje x para mejor visualización
```
...
# --- Parámetros de la simulación ---
a <- 5  # Grados de libertad del numerador
b <- 10 # Grados de libertad del denominador
num_simulaciones <- 20000

# --- Simulación ---
set.seed(456)
# 1. Generar una muestra de F(a, b)
muestra_F_ab <- rf(num_simulaciones, df1 = a, df2 = b)

# 2. Calcular el recíproco de cada valor
muestra_reciproca <- 1 / muestra_F_ab

# --- Visualización y Comparación ---
# 3. Graficar el histograma de la muestra transformada
ggplot(data.frame(x = muestra_reciproca), aes(x = x)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50,
                 fill = "lightgreen", color = "black", alpha = 0.7) +
  # 4. Superponer la PDF teórica de F(b, a)
  stat_function(
    fun = df,
    args = list(df1 = b, df2 = a), # Parámetros intercambiados
    color = "blue",
    linewidth = 1.5
  ) +
  labs(
    title = "Verificación: X ~ F(a, b) => 1/X ~ F(b, a)",
    subtitle = paste("Simulación para F(", a, ",", b, "). La curva azul es la PDF teórica de F(", b, ",", a, ")."),
    x = "Valor de 1/X",
    y = "Densidad"
  ) +
  theme_minimal() +
...
# --- Parámetros de la simulación ---
a <- 8  # Grados de libertad para la primera Chi-cuadrada (numerador)
b <- 12 # Grados de libertad para la segunda Chi-cuadrada (denominador)
num_simulaciones <- 20000

# --- Simulación ---
set.seed(789)
# 1. Generar muestras independientes de dos distribuciones Chi-cuadrada
X <- rchisq(num_simulaciones, df = a)
Y <- rchisq(num_simulaciones, df = b)

# 2. Calcular la variable F según la fórmula
muestra_F_calculada <- (X / a) / (Y / b)

# --- Visualización y Comparación ---
# 3. Graficar el histograma de la muestra transformada
ggplot(data.frame(x = muestra_F_calculada), aes(x = x)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50,
                 fill = "orange", color = "black", alpha = 0.7) +
  # 4. Superponer la PDF teórica de F(a, b)
  stat_function(
    fun = df,
    args = list(df1 = a, df2 = b),
    color = "darkred",
    linewidth = 1.5
  ) +
  labs(
    title = "Verificación: (X/a)/(Y/b) ~ F(a, b) donde X ~ χ²(a), Y ~ χ²(b)",
    subtitle = paste("Simulación para F(", a, ",", b, "). La curva roja es la PDF teórica."),
    x = "Valor de la variable F calculada",
    y = "Densidad"
  ) +
  theme_minimal() +
  coord_cartesian(xlim = c(0, 5))
