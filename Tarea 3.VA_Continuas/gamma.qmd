...
```{r}
#| label: exr-TCL_gamma-solucion-corregida
#| code-fold: true
#| fig-cap: "Verificación del Teorema Central del Límite (TCL) para $\text{Gamma}(2.5, 1.8)$"
#| fig-width: 10
#| fig-height: 7

# CARGAR explícitamente patchwork para la función wrap_plots
library(patchwork) 

alpha1 <- 2.5; lambda1 <- 1.8
k_sim1 <- 1000
n_sizes <- c(5, 10, 20, 30, 50, 100)

# Parámetros Teóricos de Gamma
media_teorica <- alpha1 / lambda1
varianza_teorica <- alpha1 / (lambda1^2)

cat("--- Resultado Ejercicio 1: TCL para Gamma ---\n")
cat("Media Teórica (mu):", round(media_teorica, 4), "\n")

results_tcl_gamma <- list()
plots_tcl <- list()

for (n in n_sizes) {
  set.seed(n) 
  medias <- replicate(k_sim1, {
    mean(rgamma(n, shape = alpha1, rate = lambda1))
  })
  
  # Parámetros observados y teóricos de la media muestral
  media_observada <- mean(medias)
  varianza_observada <- var(medias)
  varianza_teorica_media <- varianza_teorica / n
  desv_std_teorica_media <- sqrt(varianza_teorica_media)
  
  results_tcl_gamma[[as.character(n)]] <- data.frame(
    n = n,
    Media_Obs = media_observada,
    Varianza_Obs = varianza_observada,
    Varianza_Teorica_Media = varianza_teorica_media
  )
  
  # Gráfico de Histograma y Curva Normal
  data_plot <- data.frame(Medias = medias)
  plots_tcl[[as.character(n)]] <- ggplot(data_plot, aes(x = Medias)) +
    geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "orange", alpha = 0.7, color = "black") +
    stat_function(fun = dnorm, args = list(mean = media_teorica, sd = desv_std_teorica_media),
...
tabla_tcl <- bind_rows(results_tcl_gamma)

cat("\nTabla de Parámetros (Verificación TCL):\n")
print(tabla_tcl %>% kable(caption = "Comparación de Parámetros de Medias Muestrales", digits = 4) %>% kable_styling())

# Organizar gráficos (USANDO wrap_plots)
plot_final1 <- wrap_plots(plots_tcl, ncol = 3) + 
  plot_annotation(title = 'Verificación del Teorema Central del Límite para Gamma')
print(plot_final1)
```
...
```{r}
#| label: exr-suma_exponenciales-solucion
#| code-fold: true
#| fig-cap: "Suma de Variables Exponenciales vs. Densidad Gamma Teórica"
#| fig-width: 10
#| fig-height: 4

lambda2 <- 1.5
n_sizes2 <- c(5, 10, 20)
k_sim2 <- 1000 # Número de sumas

cat("--- Resultado Ejercicio 2: Suma de Exponenciales ---\n")
plots_suma_exp <- list()

for (n_exp in n_sizes2) {
  
  # Simulación: Suma de n_exp exponenciales
  set.seed(n_exp)
  sumas_exp <- replicate(k_sim2, {
    sum(rexp(n_exp, rate = lambda2))
  })
  
  # Distribución Gamma teórica: Gamma(shape=n_exp, rate=lambda2)
  data_plot2 <- data.frame(Suma = sumas_exp)
  
  plots_suma_exp[[as.character(n_exp)]] <- ggplot(data_plot2, aes(x = Suma)) +
    geom_histogram(aes(y = after_stat(density)), bins = 40, fill = "purple", alpha = 0.7, color = "black") +
    stat_function(fun = dgamma, args = list(shape = n_exp, rate = lambda2),
                  color = "green", linetype = "dashed", linewidth = 1) +
    labs(title = paste("Suma de", n_exp, "Exponenciales(1.5)"), 
         subtitle = paste0("Teórica: Gamma(", n_exp, ", 1.5)"),
         x = "Valor de la Suma (Y)", y = "Densidad") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5))
}

# Organizar gráficos
plot_final2 <- wrap_plots(plots_suma_exp, ncol = 3) + 
  plot_annotation(title = 'Suma de Exponenciales vs. Distribución Gamma Teórica')
print(plot_final2)

cat("\nConclusión sobre la forma:\n")
cat("A medida que 'n' (el número de exponenciales sumadas) aumenta (de 5 a 20),\n")
cat("la distribución Gamma se vuelve más simétrica y menos sesgada a la derecha, acercándose a la forma de una distribución Normal.\n")
```
