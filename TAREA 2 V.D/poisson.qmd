...
```{r}
#| label: exr-centro-atencion-solucion
#| code-fold: true

lambda1 <- 6 # Tasa base: 6 llamadas por minuto

cat("--- Resultados Ejercicio 1: Centro de Atención Telefónica ---\n")

# a) P(X=4) en 1 minuto (lambda=6)
prob_a <- dpois(x = 4, lambda = lambda1)
cat("a) P(4 llamadas en 1 min):", round(prob_a, 4), "\n")

# b) P(X=0) en 30 segundos (lambda = 6 * 0.5 = 3)
lambda_b <- lambda1 * 0.5
prob_b <- dpois(x = 0, lambda = lambda_b)
cat("b) P(0 llamadas en 30 s) [lambda=3]:", round(prob_b, 4), "\n")

# c) Número esperado en 1 hora (lambda = 6 * 60 = 360)
lambda_c <- lambda1 * 60
cat("c) Número esperado en 1 hora:", lambda_c, "\n")

# d) P(8 <= X <= 12) en 2 minutos (lambda = 6 * 2 = 12)
lambda_d <- lambda1 * 2
# P(X <= 12) - P(X <= 7)
prob_d <- ppois(q = 12, lambda = lambda_d) - ppois(q = 7, lambda = lambda_d)
cat("d) P(8 a 12 llamadas en 2 min) [lambda=12]:", round(prob_d, 4), "\n")
```
...
cat("--- Resultados Ejercicio 2: Línea de Producción ---\n")

# a) P(X=8) en 500 productos (lambda = 1.5 * 500/100 = 7.5)
lambda_a <- lambda_base * (n_prod / n_base)
prob_a <- dpois(x = 8, lambda = lambda_a)
cat("a) P(8 defectos en 500 prod) [lambda=7.5]:", round(prob_a, 4), "\n")

# b) P(X >= 10) en 500 productos = 1 - P(X <= 9)
prob_b <- 1 - ppois(q = 9, lambda = lambda_a)
cat("b) P(>= 10 defectos en 500 prod):", round(prob_b, 4), "\n")

# c) Hallar N tal que P(X >= 1) = 0.95 -> 1 - P(X=0) = 0.95 -> P(X=0) = 0.05
# P(X=0) = exp(-lambda) = 0.05 -> lambda = -ln(0.05)
lambda_c_req <- -log(0.05)
n_c <- lambda_c_req * n_base / lambda_base
cat("c) Productos necesarios (N) para P(X>=1)=0.95:", ceiling(n_c), "\n")

# d) Nueva tasa lambda_base_new = 1.2. P(X >= 10) en 500 productos
lambda_base_new <- 1.2
lambda_d <- lambda_base_new * (n_prod / n_base) # lambda = 6
prob_d <- 1 - ppois(q = 9, lambda = lambda_d)
cat("d) P(>= 10 defectos en 500 prod) [lambda=6]:", round(prob_d, 4), "\n")
```
...
```{r}
#| label: exr-hospital-solucion
#| code-fold: true

lambda_h <- 2.5 # Tasa base: 2.5 pacientes por hora

cat("--- Resultados Ejercicio 3: Servicio de Urgencias ---\n")

# a) P(X=3) en 1 hora (lambda=2.5)
prob_a <- dpois(x = 3, lambda = lambda_h)
cat("a) P(3 pacientes en 1 hora):", round(prob_a, 4), "\n")

# b) P(X=0) en 2 horas (lambda = 2.5 * 2 = 5)
lambda_b <- lambda_h * 2
prob_b <- dpois(x = 0, lambda = lambda_b)
cat("b) P(0 pacientes en 2 horas) [lambda=5]:", round(prob_b, 4), "\n")

# c) P(X > 8) en 4 horas (lambda = 2.5 * 4 = 10)
lambda_c <- lambda_h * 4
# P(X > 8) = 1 - P(X <= 8)
prob_c <- 1 - ppois(q = 8, lambda = lambda_c)
cat("c) P(Superar 8 pacientes en 4 horas) [P(X > 8)]:", round(prob_c, 4), "\n")

# d) Capacidad mínima (k) para P(X > k) < 0.10 en 4 horas (lambda=10)
# P(X <= k) > 0.90
capacidad_d <- qpois(p = 0.90, lambda = lambda_c)
cat("d) Capacidad mínima para P(saturación) < 10% (P(X<=k)>0.90):", capacidad_d, "\n")
```
...
```{r}
#| label: exr-tcl_poisson-solucion
#| code-fold: true
#| fig-cap: "Verificación del Teorema del Límite Central para Poisson(7)"
#| fig-width: 8
#| fig-height: 6

lambda_tcl <- 7
n_muestra <- 30 # Tamaño de la muestra
k_sim <- 1000  # Número de medias muestrales

# Simular medias muestrales
set.seed(456)
medias_muestrales <- replicate(k_sim, {
  muestra <- rpois(n_muestra, lambda = lambda_tcl)
  mean(muestra)
})

# Parámetros teóricos de la media muestral X_bar ~ N(mu, sigma^2/n)
# Para Poisson: mu = lambda, sigma^2 = lambda
media_teorica <- lambda_tcl
varianza_teorica_media <- lambda_tcl / n_muestra
desv_std_teorica_media <- sqrt(varianza_teorica_media)

# Crear data frame para el histograma
hist_data <- data.frame(Medias = medias_muestrales)

# Gráfico de Histograma y Curva Normal
plot4 <- ggplot(hist_data) +
  geom_histogram(aes(x=Medias, after_stat(density)), bins = 30, 
                 fill = "lightblue", alpha = 0.7, color = "black") +
  # Curva de densidad observada
  geom_density(aes(x = Medias), color = "red", linewidth = 1) +
  # Curva Normal Teórica
  stat_function(fun = dnorm, args = list(mean = media_teorica, 
                                         sd = desv_std_teorica_media),
                color = "blue", linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = media_teorica, 
             color = "black", linetype = "dotted", linewidth = 1) +
  labs(title = "Distribución de Medias Muestrales de Poisson",
       subtitle = paste("TCL para Poisson(7) | n =", n_muestra, " | k =", k_sim),
       x = "Media Muestral",
       y = "Densidad") +
  theme_minimal() +
...
media_observada <- mean(medias_muestrales)
varianza_observada <- var(medias_muestrales)

cat("Media Teórica (lambda):", media_teorica, "\n")
cat("Media Observada (simulación):", round(media_observada, 4), "\n")
cat("Varianza Teórica (lambda/n):", round(varianza_teorica_media, 4), "\n")
cat("Varianza Observada (simulación):", round(varianza_observada, 4), "\n")
```
...
```{r}
#| label: exr-prop_aditiva-solucion
#| code-fold: true
#| fig-cap: "Verificación de la Propiedad Aditiva de Poisson"
#| fig-width: 8
#| fig-height: 6

lambda_x1 <- 3.5
lambda_x2 <- 2.8
lambda_suma_teorica <- lambda_x1 + lambda_x2 # 6.3
k_sim <- 10000

cat("--- Resultados Ejercicio 5: Propiedad Aditiva ---\n")

# a) Generar 10,000 muestras y calcular su suma
set.seed(123)
X1 <- rpois(k_sim, lambda = lambda_x1)
X2 <- rpois(k_sim, lambda = lambda_x2)
Z <- X1 + X2 # Z = X1 + X2

# b) Comparar estadísticas de la suma con Poisson(6.3)
# Parámetros teóricos de Z ~ Poisson(6.3)
media_z_teorica <- lambda_suma_teorica
varianza_z_teorica <- lambda_suma_teorica

# Parámetros observados de Z
media_z_observada <- mean(Z)
varianza_z_observada <- var(Z)

tabla_comp_stats <- data.frame(
  Parámetro = c("Media", "Varianza"),
  Teórico_Z_Pois6.3 = c(media_z_teorica, varianza_z_teorica),
  Observado_Z_X1_X2 = c(media_z_observada, varianza_z_observada)
)

cat("b) Comparación de Estadísticas (X1+X2 vs. P(6.3)):\n")
print(tabla_comp_stats %>% kable(caption = "Comparación de Parámetros", digits = 4) %>% kable_styling())

# c) Extensión a la suma de 4 variables
lambda_x3 <- 1.0; lambda_x4 <- 1.5
lambda_total_teorica <- lambda_suma_teorica + lambda_x3 + lambda_x4 # 8.8

X3 <- rpois(k_sim, lambda = lambda_x3)
X4 <- rpois(k_sim, lambda = lambda_x4)
W <- X1 + X2 + X3 + X4 # W = X1 + X2 + X3 + X4

media_w_observada <- mean(W)
media_w_teorica <- lambda_total_teorica

cat("\nc) Extensión a 4 Variables (W ~ Poisson(8.8)):\n")
cat("   Media Teórica E(W):", media_w_teorica, "\n")
cat("   Media Observada E(W):", round(media_w_observada, 4), "\n")
```
...
```{r}
#| label: exr-aplicacion_web-solucion
#| code-fold: true

lambda_web <- 15 # Tasa base: 15 visitantes por minuto

cat("--- Resultados Ejercicio 6: Aplicación Web ---\n")

# a) P(X > 25) en 1 minuto = 1 - P(X <= 25)
prob_a <- 1 - ppois(q = 25, lambda = lambda_web)
cat("a) P(Degradación) [P(X > 25)]:", round(prob_a, 4), "\n")

# b) Duplicar capacidad (X > 50). P(X > 50)
prob_b <- 1 - ppois(q = 50, lambda = lambda_web)
cat("b) P(Degradación) con Capacidad 50:", round(prob_b, 4), "\n")

# c) Capacidad óptima (k) para P(X > k) < 0.01 -> P(X <= k) > 0.99
capacidad_c <- qpois(p = 0.99, lambda = lambda_web)
cat("c) Capacidad óptima para P(degradación) < 1%:", capacidad_c, "\n")

# d) Simulación durante 60 minutos
n_minutos <- 60
capacidad_sim <- 25
set.seed(789)
llamadas_por_minuto <- rpois(n_minutos, lambda = lambda_web)
minutos_con_degradacion <- sum(llamadas_por_minuto > capacidad_sim)
porcentaje_degradacion <- (minutos_con_degradacion / n_minutos)

cat("d) Porcentaje de tiempo con degradación (simulación 1 hora):", 
    scales::percent(porcentaje_degradacion, accuracy = 0.01), "\n")
```
...
```{r}
#| label: exr-inventarios-solucion
#| code-fold: true
...
cat("--- Resultados Ejercicio 7: Inventarios ---\n")

# a) P(Q=12) P(agotarse) = P(X > 12) = 1 - P(X <= 12)
Q_a <- 12
prob_agotarse_a <- 1 - ppois(q = Q_a, lambda = lambda_inv)
cat("a) P(agotarse con 12 unidades) [P(X > 12)]:", round(prob_agotarse_a, 4), "\n")

# b) Unidades (k) para P(X > k) < 0.05 -> P(X <= k) > 0.95
Q_b <- qpois(p = 0.95, lambda = lambda_inv)
cat("b) Unidades necesarias (k) para P(agotarse) < 5%:", Q_b, "\n")

# c) Costo esperado diario C(Q) = C_adquisicion + C_inventario_esperado + C_perdida_esperado
# Se calculará para el inventario Q=12
Q_c <- 12
C_adquisicion_c <- Q_c * costo_adquisicion # $120
# C_inventario_esperado = C_mantener * E(Q - X | X < Q) = C_mantener * (Q - E[X]) + C_mantener * sum_{x=Q+1}^inf (x-Q)P(X=x)
# Pero es más fácil C_inventario = C_mantener * E[max(Q-X, 0)]
# C_perdida_esperado = C_perdida * E[max(X-Q, 0)]

# Función de costo esperado para cualquier Q
costo_total_esperado <- function(Q, lambda) {
  # Valores posibles de ventas x para la simulación
  x_max <- qpois(0.9999, lambda) # Cortar la cola de la distribución
  x_valores <- 0:x_max
  probabilidades <- dpois(x_valores, lambda)
  
  # Costo de adquisición
  C_adquisicion_q <- Q * costo_adquisicion
  
  # Costo de mantener (Q - X cuando X < Q, i.e. max(Q-X, 0))
  C_mantener_x <- costo_mantener * pmax(Q - x_valores, 0)
  E_mantener <- sum(C_mantener_x * probabilidades)
  
  # Costo de venta perdida (X - Q cuando X > Q, i.e. max(X-Q, 0))
  C_perdida_x <- costo_perdida * pmax(x_valores - Q, 0)
  E_perdida <- sum(C_perdida_x * probabilidades)
  
  C_total <- C_adquisicion_q + E_mantener + E_perdida
  return(C_total)
}

costo_esperado_c <- costo_total_esperado(Q = Q_c, lambda = lambda_inv)
cat("c) Costo Esperado para Q=12:", scales::dollar(costo_esperado_c), "\n")

# d) Nivel óptimo de inventario
Q_posibles <- 1:20 # Rango razonable para lambda=8
costos <- sapply(Q_posibles, costo_total_esperado, lambda = lambda_inv)
Q_optimo <- Q_posibles[which.min(costos)]
costo_minimo <- min(costos)

# Mostrar tabla y resultado
tabla_costos <- data.frame(
  Inventario_Q = Q_posibles,
  Costo_Esperado = round(costos, 2)
)

cat("d) Nivel Óptimo de Inventario y Costo Mínimo:\n")
print(tabla_costos %>% filter(Inventario_Q %in% c(5, Q_optimo-1, Q_optimo, Q_optimo+1, 15)) %>% 
        kable(caption = "Costos Esperados Diarios", digits = 2) %>% 
        kable_styling())

cat("   Nivel óptimo Q:", Q_optimo, "\n")
cat("   Costo mínimo esperado:", scales::dollar(costo_minimo), "\n")
```
