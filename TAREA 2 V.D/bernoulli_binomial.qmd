...
-   La probabilidad de obtener una pieza no defectuosa es $0.98$.

    b)  Calcule la media y varianza de esta distribución

-   La media de esta distribución Bernoulli es $0.02$ y la varianza es $0.0196$.

    c)  Si se inspeccionan 10 piezas independientemente, ¿cuál es la probabilidad de encontrar exactamente 2 defectuosas?
    la probabilidad de encontrar exactamente 2 piezas defectuosas en 10 inspecciones (Binomial) es de $0.01531$ (aproximadamente $1.53\%$).
...
-   
```{r}
#| label: exr-examen-solucion
#| code-fold: true

p2 <- 1/4 # P(acertar)

cat("--- Parámetros Bernoulli (Examen) ---\n")
# a) Modelo: X ~ Bernoulli(p = 0.25)

# b) Probabilidad de acertar
cat("b) Probabilidad de acertar (p):", p2, "\n")

# c) Parámetros estadísticos
media_bernoulli_ex2 <- p2
varianza_bernoulli_ex2 <- p2 * (1 - p2)
cat("c) Media (mu):", media_bernoulli_ex2, "\n")
cat("c) Varianza (sigma^2):", varianza_bernoulli_ex2, "\n")
```
...
:::
```{r}
#| label: exr-binomial-monedas-solucion
#| code-fold: true

n3 <- 15 # lanzamientos
p3 <- 0.5 # P(cara)

cat("--- Resultados Binomial (Monedas) ---\n")
# a) P(X=8)
prob_bin_a3 <- dbinom(x = 8, size = n3, prob = p3)
cat("a) P(X=8):", round(prob_bin_a3, 4), "\n")

# b) P(X >= 12) = 1 - P(X <= 11)
prob_bin_b3 <- 1 - pbinom(q = 11, size = n3, prob = p3)
cat("b) P(X >= 12):", round(prob_bin_b3, 4), "\n")

# c) Media y Desviación estándar
media_bin3 <- n3 * p3
desv_std_bin3 <- sqrt(n3 * p3 * (1 - p3))
cat("c) Media (mu):", media_bin3, "\n")
cat("c) Desviación estándar (sigma):", round(desv_std_bin3, 4), "\n")
```
...
```{r}
#| label: exr-binomial-medicamento-solucion
#| code-fold: true
...
# a) P(X=18)
prob_bin_a4 <- dbinom(x = 18, size = n4, prob = p4)
cat("a) P(X=18):", round(prob_bin_a4, 4), "\n")

# b) P(15 <= X <= 19) = P(X <= 19) - P(X <= 14)
prob_bin_b4 <- pbinom(q = 19, size = n4, prob = p4) - pbinom(q = 14, size = n4, prob = p4)
cat("b) P(15 <= X <= 19):", round(prob_bin_b4, 4), "\n")

# c) Esperanza E(X)
esperanza_bin4 <- n4 * p4
cat("c) Esperanza E(X):", esperanza_bin4, "\n")
```
...
```{r}
#| label: exr-simulacion_5-solucion
#| code-fold: true
#| fig-cap: "Verificación del Teorema de Adición: Simulación de X+Y ~ Bin(12, 0.3)"
#| fig-width: 8
#| fig-height: 6

# Parámetros
n_x <- 5; n_y <- 7; p_sim <- 0.3
k_muestra_sim <- 10000

# 1. Simulación
set.seed(123)
X <- rbinom(k_muestra_sim, size = n_x, prob = p_sim)
Y <- rbinom(k_muestra_sim, size = n_y, prob = p_sim)
Z <- X + Y 

# 2. Parámetros teóricos de Z
n_z_teorico <- n_x + n_y # n+m = 12
p_z_teorico <- p_sim # p = 0.3
media_z_teorica <- n_z_teorico * p_z_teorico

# 3. Parámetros observados
media_z_observada <- mean(Z)

# 4. Gráfico y comparación
z_data <- data.frame(Z = Z)
dist_teorica <- data.frame(
  Valor = 0:n_z_teorico,
  Probabilidad = dbinom(0:n_z_teorico, size = n_z_teorico, prob = p_z_teorico)
)

# library(ggplot2)
simulacion_plot <- ggplot(z_data, aes(x=Z, y=after_stat(density))) +
  geom_histogram(bins = n_z_teorico + 1, fill = "lightblue", alpha = 0.7, color = "black") +
  geom_point(data = dist_teorica, aes(x = Valor, y = Probabilidad), 
             color = "red", size = 3) +
  geom_segment(data = dist_teorica, aes(x = Valor, xend = Valor, y = 0, yend = Probabilidad), 
               color = "red", linetype = "dashed") +
  labs(title = paste("Distribución de Z = X + Y (n_sim =", k_muestra_sim, ")"),
       subtitle = "Teorema de Adición de Binomiales: Z ~ Bin(12, 0.3)",
       x = "Número de Éxitos (Z)",
       y = "Densidad/Probabilidad") +
  scale_x_continuous(breaks = 0:n_z_teorico) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))
print(simulacion_plot)

cat("La distribución teórica de Z = X + Y es Bin(", n_z_teorico, ", ", p_z_teorico, ")\n")
cat("Media teórica E(Z):", media_z_teorica, "\n")
cat("Media observada de la simulación:", round(media_z_observada, 4), "\n")
```
