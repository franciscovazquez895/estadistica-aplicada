...
```{r}
#| label: exr-grafico-solucion
#| code-fold: true
#| fig-cap: "Comparación: Suma de Geométricas vs. Binomial Negativa (r=4, p=0.3)"

comparar_geometria_bn <- function(r, p, n_sim) {
  
  # 1. Simulación de la suma de r Geométricas (rgeom cuenta fallos antes del 1er éxito)
  set.seed(42) 
  sim_geometrica <- replicate(n_sim, {
    sum(rgeom(r, prob = p))
  })
  
  # 2. Simulación de la Binomial Negativa (rnbinom cuenta fallos antes del r-ésimo éxito)
  sim_bn <- rnbinom(n_sim, size = r, prob = p)
  
  # 3. Crear Data Frame para ggplot
  data_sim <- data.frame(
    Fallas = c(sim_geometrica, sim_bn),
    Distribucion = factor(c(rep("Suma de Geométricas", n_sim), 
                           rep("Binomial Negativa", n_sim)))
  )
  
  # 4. Gráfico de Histograma Comparativo
  max_fallas <- max(data_sim$Fallas)
  
  plot <- ggplot(data_sim, aes(x = Fallas, fill = Distribucion)) +
    geom_histogram(aes(y = after_stat(density)), 
                   position = "identity", bins = max_fallas + 1, alpha = 0.6) +
    labs(title = paste("Comparación: Suma de Geométricas vs. Binomial Negativa"),
         subtitle = paste("Parámetros: r =", r, ", p =", p, ", n_sim =", n_sim),
         x = "Número de Fallas (k)",
         y = "Densidad Observada") +
    scale_x_continuous(breaks = seq(0, max_fallas, by = max(1, floor(max_fallas/10)))) +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5),
          legend.position = "bottom")
  
  # Crear Tabla de Frecuencias (ejemplo para mostrar la equivalencia)
  freq_geom <- prop.table(table(sim_geometrica))
  freq_bn <- prop.table(table(sim_bn))
  
  valores_comunes <- as.numeric(union(names(freq_geom), names(freq_bn)))
  
  tabla_comparativa <- data.frame(
    Fallas = valores_comunes,
    Geom = freq_geom[as.character(valores_comunes)],
    BN = freq_bn[as.character(valores_comunes)]
  ) %>% replace(is.na(.), 0) %>%
    mutate(Fallas = as.integer(Fallas)) %>%
    arrange(Fallas)
  
  cat("\nTabla de Frecuencias Relativas (primeras filas):\n")
  print(head(tabla_comparativa) %>% kable(caption = "Frecuencias Relativas Simulación", digits = 4) %>% kable_styling())
  
  return(plot)
}

# Ejecutar con r=4 (éxitos requeridos) y p=0.3 (prob. de éxito)
plot1 <- comparar_geometria_bn(r = 4, p = 0.3, n_sim = 10000)
print(plot1)
```
...
```{r}
#| label: exr-baloncesto-solucion
#| code-fold: true
#| fig-cap: "Distribución de Fallos en Tiros Libres (BN: r=8, p=0.7)"

r2 <- 8   # Éxitos requeridos
p2 <- 0.7 # P(éxito)

# a) Modelación
cat("a) El problema se modela como la suma de 8 variables geométricas, donde X ~ Binomial Negativa (r=8, p=0.7) contando el número de fallos.\n")

# b) Número esperado de fallos E(X) = r * (1-p) / p
esperanza_fallos <- r2 * (1 - p2) / p2
cat("b) Número esperado de fallos E(X):", round(esperanza_fallos, 4), "\n")

# c) Simulación de 1000 sesiones y comparación
n_sim2 <- 1000
set.seed(2)
fallos_simulados <- rnbinom(n_sim2, size = r2, prob = p2)
media_observada <- mean(fallos_simulados)
cat("c) Media observada de la simulación (n_sim=1000):", round(media_observada, 4), "\n")

# Gráfico de la simulación
data_sim2 <- data.frame(Fallas = fallos_simulados)

plot2 <- ggplot(data_sim2, aes(x=Fallas)) +
  geom_histogram(aes(y = after_stat(density)), bins = max(fallos_simulados)+1, 
                 fill = "coral", alpha = 0.7, color = "black") +
  geom_vline(xintercept = esperanza_fallos, color = "blue", linetype = "dashed", linewidth = 1) +
  labs(title = paste("Distribución de Fallos en Tiros Libres (BN: r=8, p=0.7)"),
       subtitle = paste("Media Teórica:", round(esperanza_fallos, 2), " | Media Observada:", round(media_observada, 2)),
       x = "Número de Fallos (k)",
       y = "Densidad Observada") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
...
```{r}
#| label: exr-biotecnologia-solucion
#| code-fold: true

r3 <- 4   # Éxitos requeridos
p3 <- 0.3 # P(éxito)

cat("--- Resultados Biotecnología ---\n")

# a) P(X=12 fallidas)
prob_a3 <- dnbinom(x = 12, size = r3, prob = p3)
cat("a) P(12 fallidas):", round(prob_a3, 4), "\n")

# b) P(X > 20 fallidas) = 1 - P(X <= 20)
prob_b3 <- 1 - pnbinom(q = 20, size = r3, prob = p3)
cat("b) P(> 20 fallidas):", round(prob_b3, 4), "\n")

# c) Costo esperado en pruebas fallidas
# E(X) = r * (1-p) / p
esperanza_fallos_c <- r3 * (1 - p3) / p3
costo_esperado <- esperanza_fallos_c * 10000
cat("c) Fallos esperados E(X):", round(esperanza_fallos_c, 2), "\n")
# Usar scales::dollar para formato de moneda (asumiendo que la librería está cargada)
cat("c) Costo esperado en pruebas fallidas:", scales::dollar(costo_esperado), "\n")
```
